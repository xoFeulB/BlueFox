...
window.customElements.define("ga-te", class extends HTMLElement {
  ...
  async load(src) {
    ...
    try {
      ...
      if (r.ok) {
        this.append(
          ...(new DOMParser()).parseFromString(await r.text(), "text/html").body.childNodes
        );
      }
    } catch (e) {
      this.innerText = "^.,.^ < Oh, something went wrong.";
    }
    await this.onGateLoad(this);
  }

  set onLoad(callback) {
    this.onGateLoad = callback;
    if (this.innerText) {
      this.onGateLoad(this);
    }
  }
  async onGateLoad(DOM) { }
